...
//! bridge**: instantiate the compiled `pathland-guest` component in wasmtime
...
impl GuestHost {
    /// Instantiate the compiled `pathland-guest` component.
...
mod tests {
    ...
    use pathland_transport::BatchDecoder;
    ...
    fn component_bytes() -> Vec<u8> {
        ...
        std::fs::read(path).expect("run `cargo component build -p pathland-guest` first")
    }
    ...
    fn bridge_to_batch_to_render_tree() {
        ...
        // Decode the batch and feed pathland-host's RenderTree.
