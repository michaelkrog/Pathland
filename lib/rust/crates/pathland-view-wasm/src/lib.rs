...
use pathland_core::{init_memory, Guest as RingGuest, Host, MemoryLayout, Opcode};
use pathland_core_transport::BatchEncoder;
...
struct State {
    ...
    /// Frame watermark already drained into a batch; used to emit a decodable
    /// empty batch when a frame completes with zero delta opcodes.
...
impl EngineGuest for State {
    ...
    fn take_batch() -> Vec<u8> {
        ...
        if bytes.is_empty() && frame_count != s.last_frame_count {
            // A frame completed with zero delta opcodes (unchanged tree): still
            // emit a decodable empty batch so the host sees a well-formed frame.
            bytes.extend(s.encoder.encode(frame_count, &[], &[]));
        }
        s.last_frame_count = frame_count;
        bytes
    }
}
